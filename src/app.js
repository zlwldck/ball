var GameLayer = cc.Layer.extend({
        ...
        for (var i = 0; i < Constant.MAP_SIZE; i++) {
            ...
            for (var j = 0; j < this.map[i].length; j++) {
                ...
                if(!candy){
                    ...
                    var failLength = missCount;
                    if(failLength > 0) {
                        var duration = Math.sqrt(2*failLength/Constant.FALL_ACCELERATION);
                        if(duration > maxTime)
                            maxTime = duration;
                        var move = cc.moveTo(duration,candy.x,candy.y-Constant.CANDY_WIDTH*failLength).easing(cc.easeIn(2));
                        candy.runAction(move);
                        candy.row -= failLength;
                        this.map[i][j] = null;
                        this.map[i][candy.row] = candy;
                    }
                }
    ...
    _checkSuccessOrFail: function() {
        ...
        }else if(this.limitStep-this.steps <= 0) {
            this.ui.shouFail();
